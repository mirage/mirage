...
(** The kind of package installation:

    - Global: installed with opam.
    - Local: locally fetched to build with dune. *)
...
val v :
  ...
  string ->
  t
(** [v ~kind ~build ~sublibs ~libs ~min ~max ~pin opam] is a [package]. [Build]
    ...
    pin-depends} is generated. [~kind] specifies the installation location of
    the package. *)

val with_kind : kind:kind -> t -> t
(** [with_kind t] returns t with chosen installation location.*)

val name : t -> string
(** [name t] is [t]'s opam name. *)

val key : t -> string
(** [key t] is [t]'s key (concatenation of name and installation kind). *)

val kind : t -> kind
(** [kind t] is [t]'s installation kind. *)
