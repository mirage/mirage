...
//MlStrings: based on buffers

//Provides: MlString
//Requires: caml_array_bound_error

Buffer.prototype.getLen  = function () {return this.length };
Buffer.prototype.get     = function (i) {return this[i];};
Buffer.prototype.safeGet = function (i) {
  if ((i<0)||(this.length<i)) caml_array_bound_error ();
  return this[i];
};
Buffer.prototype.set    = function (i, c) {this[i] <- c; return 0;};
Buffer.prototype.safeSet= function (i, c) {
  if ((i<0)||(this.length<i)) caml_array_bound_error ();
  this[i] <- c;
return 0;
};
...
//Blunt inheritance XXX: is this really a good idea?
MlString = Buffer;
MlWrappedString = Buffer;
MlMakeString = Buffer;
MlStringFromArray = Buffer;
...
//Requires: MlString
function MlStringFromArray (a) { new MlString (a); }
...
//Requires: MlString
function caml_fill_string(s, i, l, c) { s.fill (c, i, l); }
...
//Requires: MlString
function caml_string_lessequal(s1, s2) { return s1 <= s2; }
//Provides: caml_string_lessthan
//Requires: MlString
function caml_string_lessthan(s1, s2) { return s1 < s2; }
//Provides: caml_string_greaterthan
//Requires: MlString
function caml_string_greaterthan(s1, s2) { return s1 > s2; }
//Provides: caml_string_greaterequal
//Requires: MlString
function caml_string_greaterequal(s1, s2) { return s1 >= s2; }
//Provides: caml_blit_string
//Requires: MlString
function caml_blit_string(s1, i1, s2, i2, len) {
  if (len === 0) return;
  if (i1 < 0 || i1 + len > s1.length || i2 < 0 || i2 + len > s2.length) {
    caml_invalid_argument ("String.blit");
    return;
  };
  var chunk = s1.toString('binary', i1, i1+len);
  var w = s2.write(chunk, i2, 'binary');
  return w;
}
//
// Js_of_ocaml runtime support
...
function () {
  ...
  return function (s, ofs, len) {
    ...
    for (var i = 0; i < len; i+=4)
      buf[i>>2] = s[i] | (s[i+1] << 8) | (s[i+2] << 16) | (s[i+3] << 24);
    for (; i < len; i++) buf[i>>2] |= s[i] << (8 * (i & 3));
...
function caml_compare_val (a, b, total) {
  ...
  if (Buffer.isBuffer(a)) {
    ...
    if (b instanceof Array && b[0] == (b[0]|0)) {
      ...
      return 1;
  } else if (Buffer.isBuffer(b) || (b instanceof Array && b[0] == (b[0]|0)))
    return -1;
  else {
    if (a < b) return -1;
    if (a > b) return 1;
...
function caml_parse_sign_and_base (s) {
  var i = 0, base = 10, sign = s[0] == 45?(i++,-1):1;
  if (s[i] == 48)
    switch (s[i + 1]) {
...
function caml_parse_format (fmt) {
  ...
  var f =
    { justify:'+', signstyle:'-', filler:' ', alternate:false,
      base:10, signedconv:false, width:0, uppercase:false,
...
function caml_hash_univ_param (count, limit, obj) {
  ...
  function hash_aux (obj) {
    ...
    if (obj instanceof Array && obj[0] == (obj[0]|0)) {
      ...
      count --;
      l = obj.length;
      for (var i = 0; i < l; i++) hash_accu = (hash_accu * 19 + obj[i]) | 0;
    } else if (obj == (obj|0)) {
      // Integer
      count --;
...
function caml_js_from_byte_string (s) {return s.toString('utf8');}
//Provides: caml_js_to_byte_string
function caml_js_to_byte_string (s) {return new Buffer(s);}
