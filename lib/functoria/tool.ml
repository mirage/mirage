...
module Make (P : S) = struct
  ...
  let re_exec t ?ppf ?err_ppf argv =
    ...
    let config_exe = Fpath.(build_dir t / "config.exe") in
    let command = Bos.Cmd.(v "dune" % "exec" % "--" % p config_exe %% args) in
  ...
  let generate_makefile ~depext ~duniverse name =
    ...
    let contents =
      Fmt.to_to_string Makefile.pp (Makefile.v ~depext ?duniverse name)
    in
  ...
  let with_alias ~save_args args ~depext ~duniverse ?ppf ?err_ppf argv =
    ...
    generate_makefile ~depext ~duniverse name >>= fun () ->
    generate_dune `Project args ?err_ppf () >>= fun () ->
    generate_dune `Workspace args ?err_ppf () >>= fun () ->
    generate_dune `Full args ?err_ppf ()

  let configure ({ args; depext; duniverse } : _ Cli.configure_args) ?ppf
      ?err_ppf argv =
    with_alias ~save_args:true args ~depext ~duniverse ?ppf ?err_ppf argv
