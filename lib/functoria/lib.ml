...
module Make (P : S) = struct
    ...
    ( Fmt.str {|%s configure%a --no-extra-repo|} P.name
      ...
      (fun sub ->
        Fmt.str {|make %a"lock" "depext-lockfile" "pull"|}
          ...
          sub),
      (fun sub unikernel ->
        Fmt.str {|%a%s/%s.opam.locked|}
          Fmt.(option ~none:(any "") Fpath.pp)
          sub P.name unikernel),
  ...
  let dune_contents alias args =
    ...
    | `Config ->
        ...
        in
        let dune =
          Dune.(v (config ~config_ml_file ~packages:P.packages @ includes))
        in
        Fmt.str "%a\n" Dune.pp dune
  ...
  let configure ({ args; extra_repo; _ } : _ Cli.configure_args) =
    ...
    let name = P.name_of_target info in
    let opam_name = Misc.Name.opamify name in
    let* _ = Action.mkdir (gen_dir args) in
    let* () =
      let gen_dir = gen_dir args in
      Action.with_dir gen_dir (fun () ->
          Log.info (fun m -> m "in dir %a" Fpath.pp gen_dir);
          (* OPAM file *)
    ...
    let* () =
      Action.with_dir build_dir (fun () ->
          Log.info (fun m -> m "in dir %a" Fpath.pp build_dir);
          (* overwrite the dune file generated by tool.ml *)
          let* () = generate_dune `Config args in
          Action.ok ())
    in
    Action.ok ()
