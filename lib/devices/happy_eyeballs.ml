...
let generic_happy_eyeballs aaaa_timeout connect_delay connect_timeout
  ...
  let packages =
    [ package "happy-eyeballs-mirage" ~min:"1.0.0" ~max:"1.1.0" ]
  in
  let runtime_args =
    runtime_args_opt
      [
  ...
  in
  let err () = connect_err "generic_happy_eyeballs" 3 ~max:8 in
  let connect _info modname = function
    | _time :: _mclock :: stack :: rest ->
        ...
        code ~pos:__POS__ {ocaml|%s.connect_device %a%a%a%a%a%a %s|ocaml}
