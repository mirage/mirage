...
let find_ld pkg =
  ...
  | Error msg ->
      Log.warn (fun m ->
          m "error %a while pkg-config %s --variable=ld, using ld"
...
let link info name target _target_debug =
  ...
  | #Mirage_key.mode_unix ->
      ...
      Bos.OS.Cmd.run link >>= fun () -> Ok name
  | #Mirage_key.mode_xen ->
      ...
      if String.is_prefix ~affix:"arm" machine then (
        ...
        Bos.OS.Cmd.run objcopy_cmd >>= fun () -> Ok out )
      else
        let link = Bos.Cmd.(linker % "-o" % out) in
        Log.info (fun m -> m "linking with %a" Bos.Cmd.pp link);
        Bos.OS.Cmd.run link >>= fun () -> Ok out
  | #Mirage_key.mode_solo5 ->
      ...
      Bos.OS.Cmd.run linker >>= fun () -> Ok out
