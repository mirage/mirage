...
let w = Package.v ~min:"1.0" ~max:"2.0" "foo" ~kind:`Global
...
let test_package_pp () =
  ...
  Alcotest.(check string) "key(x)" (Package.key x) "local-foo";
  Alcotest.(check string) "key(w)" (Package.key w) "global-foo"
