...
(** The type for configuration keys. Keys are used to retrieve the
    ...
    autogenerated [Bootgen_var] module) but also to parametrize the
    choice of {{!Functoria_dsl.if_impl}module implementation}. *)
...
(** The type for data dependencies. Data dependencies form a directed
    ...
    {!values}. Vertices of the graph hold {{!value}values}. Edges
    represent evaluation-order (e.g. data) dependencies between
    values. *)
