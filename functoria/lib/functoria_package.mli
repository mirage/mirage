...
(** [v ~build ~sublibs ~libs ~min ~max ~pin opam] is a [package]. [Build]
    ...
    library names [ ["foo"; "foo.mirage"] ].  In case the library name is
    disjoint (or empty), use [~libs].  Specifying both [~libs] and
...
(** [libraries t] is the set of libraries (and sub-libraries) used in
    the package [t]. For most packages, it will only contain one
    library whose name is [name t]. *)
...
(** [merge x y] is merges the information of [x] and [y]. The result is
    [None] if [opam x != opam y]. *)
