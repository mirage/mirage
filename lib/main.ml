...
let configure =
  ...
  let configure compiler no_install file = Mirari.configure ?compiler ~no_install ~file in
  Term.(pure configure $ switch $ no_install $ file), term_info "configure" ~doc ~man
...
let build =
  ...
  let build compiler file = Mirari.build ?compiler ~file in
  Term.(pure build $ switch $ file), term_info "build" ~doc ~man
...
let clean =
  ...
  let clean compiler file = Mirari.clean ?compiler ~file in
  Term.(pure clean $ switch $ file), term_info "clean" ~doc ~man
