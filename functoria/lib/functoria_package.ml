...
let merge a b =
  if a.name <> b.name then None else
  let name = a.name in
...
let v ?(build = false) ?sublibs ?libs ?min ?max ?pin name =
  let libs = match sublibs, libs with
    | None, None -> [name]
    | Some xs, None -> name :: List.map (fun x -> name ^ "." ^ x) xs
    | None, Some a -> a
    | Some _, Some _ ->
      Fmt.invalid_arg
        "dependent package %s may either specify ~sublibs or ~ocamlfind"
        name
  in
...
let pp ?(surround="") ppf p =
  Fmt.pf ppf "%s%s%s %s"
    surround p.name surround (exts_to_string p.min p.max p.build)
