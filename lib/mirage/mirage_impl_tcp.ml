...
let tcpv4v6_socket_conf ipv4_key ipv6_key =
  let keys = [ Key.v ipv4_key; Key.v ipv6_key ] in
...
let socket_tcpv4v6 ?group ipv4 ipv6 =
  ...
  and ipv6 =
    ...
    | Some ip -> Some (Ipaddr.V6.Prefix.make 128 ip)
  in
  tcpv4v6_socket_conf (Key.V4.network ?group ipv4) (Key.V6.network ?group ipv6)
