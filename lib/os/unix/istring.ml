...
module View = struct
  ...
  let to_byte t off : byte = int_of_char (to_char t off)

  (* Get a uint16 out of the view.
     TODO: big-endian only on the wire. *)
  let to_uint16_be t off : uint16 = Raw.to_uint16_be t.i off 

  (* Get a uint32 out of the view.
     TODO: big-endian only on the wire. *)
  let to_uint32_be t off = Raw.to_uint32_be t.i off 

  (* Get a uint64 out of the view.
     TODO: big-endian only on the wire. *)
  let to_uint64_be t off = Raw.to_uint64_be t.i off 

  (* Unmarshalling position functions *)

  (* Skip forward a number of bytes *)
  let skip env x = env.pos <- env.pos + x
