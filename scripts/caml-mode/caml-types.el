...
(defun caml-types-locate-type-file (target-path)
  (let ((backends (list "" "/unix-socket" "/unix-direct" "/node" "/xen"))
        (sibling (concat (file-name-sans-extension target-path) ".annot")))
    (if (file-exists-p sibling)
        sibling
        (let ((project-dir (file-name-directory sibling))
              (type-path nil))
          (message "foo")
          (while (null type-path)
            ...
            (dolist (key backends)
              (let ((file (expand-file-name
                           ...
                           (expand-file-name (concat "_build" key) project-dir))))
