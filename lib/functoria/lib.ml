...
module Make (P : S) = struct
  ...
  let dune_contents alias args =
    ...
    | `Build ->
        ...
        let config_ml_file = args.Cli.config_file in
        let config = Dune.config ~config_ml_file ~packages:P.packages in
        let install = Key.eval (Info.context info) (Engine.install info jobs) in
        let install =
          Install.dune ~context_name_for_bin:(P.context_name info)
            ~context_name_for_etc:"default" install ~subdir:"dist"
        in
        let dune =
          Dune.v (config @ copy_files files @ Engine.dune info jobs @ install)
        in
        Fmt.str "%a\n" Dune.pp dune
    | `Project ->
  ...
  let configure
      ({ args; project_files; depext; extra_repo; _ } : _ Cli.configure_args) =
    ...
    if not project_files then Action.ok ()
    else
      let* () =
      ...
      in
      let* () =
        (* overwrite with config+build+dist rules *)
        Action.with_dir build_dir (fun () -> generate_dune `Build args)
      in
