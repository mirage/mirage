...
let _ =
  ...
  (* check if app.mir exists, otherwise, create it *)
  let () =
    let mir = sprintf "%s/app.mir" build_dir in
    if not (Sys.file_exists mir) then begin
      let all = Array.to_list (Sys.readdir build_dir) in
      let mls = List.filter (fun str -> Filename.check_suffix str ".ml") all in
      let names = List.map (fun str -> Filename.chop_suffix str ".ml") mls in
      let modules = List.map String.capitalize names in
      let out = open_out mir in
      output_string out "# Autogenerated by mir\n";
      List.iter (fun m ->
        output_string out m;
        output_string out "\n")
        modules;
